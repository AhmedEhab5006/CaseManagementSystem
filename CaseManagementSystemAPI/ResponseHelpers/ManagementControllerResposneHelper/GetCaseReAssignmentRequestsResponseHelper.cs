...
{
    public class GetCaseReAssignmentRequestsResponseHelper
    {
        public static IActionResult Map(PagedResult<CaseReAssignmentRequestGetDto> result)
        {
            return result switch
            {
                not null => new OkObjectResult(
                    new APIResponseHandler<PagedResult<CaseReAssignmentRequestGetDto>>(
                        200,
                        "Success",
                        data: result
                    )
